...
export interface ComputableTrade {
  id: string
...
export function computeStatistics(trades: ComputableTrade[]): StatisticsResult {
  const pnlTrades = trades.map(t => ({ pnl: t.pnl }))
  ...
  for (const t of trades) {
    ...
    tickerMap.get(instr)!.push({ pnl: t.pnl })
  }
  ...
  for (const t of trades) {
    ...
    dateMap.get(key)!.push({ pnl: t.pnl })
  }
  ...
  for (const t of trades) {
    ...
    for (const tag of journal.customTags) {
      if (KNOWN_TIMEFRAMES.has(tag)) {
        ...
        timeframeMap.get(tag)!.push({ pnl: t.pnl })
      } else {
        if (!tagMap.has(tag)) tagMap.set(tag, [])
        tagMap.get(tag)!.push({ pnl: t.pnl })
      }
    }
  }
  ...
  for (const t of trades) {
    ...
    weekdayMap.get(dayIdx)!.push({ pnl: t.pnl })
  }
  ...
  const grandResult = computeRR(pnlTrades)
  ...
  for (const t of trades) {
    const d = extractDate(t.entryDate)
    const key = d.toISOString().slice(0, 10)
    dayPnlMap.set(key, (dayPnlMap.get(key) || 0) + t.pnl)
  }
  ...
  const grossProfit = grandResult.grossWin
  const grossLoss = grandResult.grossLoss
  const winningTrades = grandResult.wins
  const losingTrades = grandResult.losses
  const avgWin = winningTrades > 0 ? grossProfit / winningTrades : 0
  const avgLoss = losingTrades > 0 ? grossLoss / losingTrades : 0
  const totalRMultiple = avgLoss > 0 ? grandPnl / avgLoss : 0
  const expectancy = trades.length > 0 ? grandPnl / trades.length : 0
