...
export default function Navigation({ locale }: NavigationProps) {
 ...
 return (
  ...
  <div className="mx-auto flex h-[72px] w-full max-w-6xl items-center justify-between rounded-full border border-border bg-black/80 px-4 shadow-lg sm:px-6">
 ...
 <span className="text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground">Obsidian</span>
  ...
  <div className="hidden rounded-full border border-border bg-muted/30 p-1 md:flex md:items-center md:gap-1">
  {navLinks.map((link) => (
  <Link
  key={link.href}
  href={link.href}
  className="rounded-full px-4 py-2 text-[13px] font-medium tracking-[-0.01em] text-muted-foreground transition-[background-color,border-color,color,opacity] duration-200 hover:bg-muted hover:text-foreground focus-visible:outline-2 focus-visible:outline-ring focus-visible:outline-offset-2"
 >
 {link.label}
 </Link>
 ))}
 ...
 <div className="hidden md:flex items-center gap-3">
  <Button variant="ghost" size="sm" className="rounded-full px-4 text-foreground/60 hover:bg-muted hover:text-foreground">
  Login
  </Button>
 ...
 <div className="flex flex-col gap-3">
  <Button variant="outline" size="lg" className="w-full rounded-full border-border bg-muted/30">
  Login
  </Button>
