...
- [ ] 5. MiniMax Navbar Component

  **What to do**:
  - Create a new `components/navbar-minimax.tsx` component following MiniMax's navigation pattern:
    - White background (`#ffffff`)
    - Qunt Edge logo left-aligned (keep existing logo)
    - Horizontal nav links in DM Sans 14px weight 500, `#18181b` text
    - Pill-shaped active indicators (9999px radius, `rgba(0,0,0,0.05)` background)
    - "Login" text link minimal right-side (keep existing auth link logic)
    - Sticky header behavior
    - Mobile: hamburger menu (MiniMax responsive pattern)
  - Include MiniMax pill-button styling:
    - Primary dark button: `#181e25` bg, white text, 8px radius
    - Nav pill: `rgba(0,0,0,0.05)` bg, `#18181b` text, 9999px radius
  - Ensure i18n support — use `useI18n()` for all nav labels
  - Ensure locale-aware links (prefix with `/${locale}`)
  - Port the auth state logic from existing `components/navbar.tsx`

  **Must NOT do**:
  - Do NOT delete `components/navbar.tsx` — dashboard may still reference it
  - Do NOT add LandingSidebar integration — it's being removed
  - Do NOT use hardcoded hex colors in className — use Tailwind/CSS variables

  **Recommended Agent Profile**:
  - **Category**: `visual-engineering`
    - Reason: Navigation component with responsive behavior, animations, and precise visual styling
  - **Skills**: [`nextjs`, `frontend-ui-ux`]
    - `nextjs`: App Router navigation patterns, locale routing
    - `frontend-ui-ux`: Clean navbar design with pill tabs, responsive hamburger
  ...
  **Pattern References**:
  - `components/navbar.tsx` — Existing navbar. Port auth state logic, nav items, and link patterns. Don't copy styling.
  - `components/marketing-layout-shell.tsx` — Shows how navbar is integrated into the marketing shell

  **API/Type References**:
  - `awesome-design-md/design-md/minimax/DESIGN.md:145-151` — Navigation styling: clean horizontal, DM Sans 14px weight 500, pill active indicators, sticky header
  - `awesome-design-md/design-md/minimax/DESIGN.md:97-125` — Button styling: pill primary dark, pill nav, pill white

  **Test References**:
  - `locales/en/*.ts` — Existing i18n keys for navigation labels

  **WHY Each Reference Matters**:
  - `navbar.tsx`: Contains auth state, nav items, and link routing logic — must be preserved in new component.
  - MiniMax DESIGN.md Sections 3-4: Exact visual specs for navbar and buttons.
  - i18n files: Ensure nav labels use existing translation keys.
  ...
  **QA Scenarios (MANDATORY):**
  ...
  ```
  Scenario: Navbar renders with MiniMax styling
    Tool: Playwright
    Steps:
      1. Navigate to `http://localhost:3000/en`
      2. Assert navbar element exists
      3. Assert navbar background is white (#ffffff)
      4. Assert nav links use DM Sans font
      5. Assert at least one pill-shaped element (border-radius: 9999px)
      6. Screenshot navbar
    Expected Result: White navbar with DM Sans text and pill elements
    Failure Indicators: Dark navbar, Geist font, sharp-cornered elements
  ...
  Scenario: Navbar mobile hamburger
    ...
    Steps:
      ...
      4. Click hamburger
      5. Assert nav menu opens
    Expected Result: Hamburger menu works on mobile
  ...
  Scenario: Navbar sticky behavior
    ...
    Steps:
      1. Navigate to `http://localhost:3000/en`
      2. Scroll down 500px
      3. Assert navbar is still visible (position: sticky)
    Expected Result: Navbar stays visible on scroll
    Evidence: .sisyphus/evidence/task-5-sticky.png
  ```

  **Commit**: YES (groups with Wave 2)
  - Message: `feat(landing): create MiniMax-style navbar component`
  - Files: `components/navbar-minimax.tsx`
  - Pre-commit: `npm run typecheck`

- [ ] 6. MiniMax Dark Footer

  **What to do**:
  - Restyle `components/footer.tsx` to match MiniMax footer pattern:
    - Dark background (`#181e25`)
    - Multi-column layout with product/company links
    - DM Sans 14px for link text
    - `rgba(255, 255, 255, 0.8)` text color for links
    - Section headers slightly brighter (`rgba(255, 255, 255, 0.9)`)
  - Keep all existing footer content (links, legal text, social icons)
  - Ensure responsive: multi-column → stacked on mobile
  - Ensure i18n support maintained

  **Must NOT do**:
  - Do NOT remove any existing footer links or content
  - Do NOT change footer structure — only visual styling

  **Recommended Agent Profile**:
  - **Category**: `visual-engineering`
    - Reason: Footer restyling with responsive multi-column layout
  - **Skills**: [`nextjs`]
    - `nextjs`: Component patterns, responsive design
  ...
  **Pattern References**:
  - `components/footer.tsx` — Existing footer component. Restyle in place.

  **API/Type References**:
  - `awesome-design-md/design-md/minimax/DESIGN.md:20` — Dark footer `#181e25`
  - `awesome-design-md/design-md/minimax/DESIGN.md:143` — On-dark text `rgba(255,255,255,0.8)`

  **WHY Each Reference Matters**:
  - `footer.tsx`: The exact component to restyle. Must keep all content/links.
  - MiniMax DESIGN.md: Footer color specs.
  ...
  **QA Scenarios (MANDATORY):**
  ...
  ```
  Scenario: Footer renders with MiniMax dark styling
    Tool: Playwright
    Steps:
      1. Navigate to `http://localhost:3000/en`
      2. Scroll to footer
      3. Assert footer background is `#181e25` or close equivalent
      4. Assert footer text color is light (rgba white)
      5. Screenshot footer
    Expected Result: Dark footer with light text, MiniMax aesthetic
    Evidence: .sisyphus/evidence/task-6-footer.png
  ...
  Scenario: Footer responsive stacking
    ...
    Steps:
      ...
      2. Navigate to `http://localhost:3000/en`
      3. Scroll to footer
      4. Assert footer columns stack vertically
    Expected Result: Single-column stacked footer on mobile
    Evidence: .sisyphus/evidence/task-6-footer-mobile.png
  ```

  **Commit**: YES (groups with Wave 2)
  - Message: `style(landing): restyle footer to MiniMax dark aesthetic`
  - Files: `components/footer.tsx`
  - Pre-commit: `npm run typecheck`

- [ ] 7. Restyle RollingAdBanner

  **What to do**:
  - Restyle `components/rolling-ad-banner.tsx` (or similar) with light theme tokens
  - Replace dark background with MiniMax-compatible styling:
    - Light surface with subtle border (`#e5e7eb`)
    - DM Sans typography
    - Brand blue (`#1456f0`) accent for CTA text
  - Keep the scrolling/rolling animation behavior
  - Ensure visibility on white background (adequate contrast)

  **Must NOT do**:
  - Do NOT remove the rolling animation functionality
  - Do NOT change the ad content source

  **Recommended Agent Profile**:
  - **Category**: `quick`
    - Reason: Single component restyle, token swap
  - **Skills**: [`nextjs`]
  ...
  **Pattern References**:
  - `components/rolling-ad-banner.tsx` — Component to restyle

  **WHY Each Reference Matters**:
  - Exact component to modify. Must understand its current styling approach.
  ...
  **QA Scenarios (MANDATORY):**
  ...
  ```
  Scenario: RollingAdBanner renders on white background
    Tool: Playwright
    Steps:
      1. Navigate to `http://localhost:3000/en`
      2. Find rolling ad banner element
      3. Assert it has light surface styling (not dark obsidian)
      4. Assert text is readable (contrast check)
      5. Verify animation is running (element position changes)
    Expected Result: Light-styled banner with working animation
    Evidence: .sisyphus/evidence/task-7-banner.png
  ```

  **Commit**: YES (groups with Wave 2)
  - Message: `style(landing): restyle rolling ad banner for light theme`
  - Files: `components/rolling-ad-banner.tsx`
  - Pre-commit: `npm run typecheck`

- [ ] 8. MarketingLayoutShell — Remove Sidebar, Integrate New Components
...
- [ ] 10. Features Component Restyle

  **What to do**:
  - Restyle `components/features.tsx` with MiniMax card patterns:
    - Section heading: Outfit 31px weight 600
    - Feature cards: white background, 20px radius, subtle shadow
    - Card titles: Outfit 28px weight 500-600
    - Feature labels: Poppins 18px weight 500
    - Purple-tinted shadow for featured cards: `rgba(44,30,116,0.16) 0px 0px 15px`
  - If features have icons/illustrations: ensure visible on white background

  **Must NOT do**:
  - Do NOT remove feature content or reorder features
  ...
  **References**:
  - `components/features.tsx` — Component to restyle
  - `awesome-design-md/design-md/minimax/DESIGN.md:75` — Section Heading: 31px Outfit weight 600
  - `awesome-design-md/design-md/minimax/DESIGN.md:126-138` — Product card styling with 20-24px radius

  **Acceptance Criteria**:

  **QA Scenarios (MANDATORY):**
  ```
  Scenario: Feature cards render with MiniMax styling
    Tool: Playwright
    Steps:
      1. Navigate to `http://localhost:3000/en`
      2. Scroll to features section
      3. Assert feature cards have white bg with rounded corners (radius ≥ 16px)
      4. Assert card titles use Outfit or DM Sans
      5. Assert subtle shadow visible on cards
      6. Screenshot features section
    Expected Result: White cards with generous radius and subtle shadows on white background
    Evidence: .sisyphus/evidence/task-10-features.png
  ```

  **Commit**: YES (groups with Wave 3)
  - Message: `style(landing): restyle features component for MiniMax light theme`
  - Files: `components/features.tsx`

- [ ] 11. FAQ Component Restyle
