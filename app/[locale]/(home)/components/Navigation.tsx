    ...
    <nav className="fixed left-0 right-0 top-0 z-50 px-4 py-3 sm:px-6">
      <div className="mx-auto flex h-[72px] w-full max-w-6xl items-center justify-between rounded-[2rem] border border-white/[0.08] bg-black/72 px-4 shadow-[0_0_0_0.5px_rgba(180,210,255,0.08),0_24px_70px_-40px_rgba(0,0,0,0.92)] backdrop-blur-2xl backdrop-saturate-200 sm:px-5">
        ...
        <div className="hidden rounded-full border border-white/[0.06] bg-white/[0.03] p-1 md:flex md:items-center md:gap-1">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="rounded-full px-4 py-2 text-[13px] font-medium tracking-[-0.01em] text-foreground/52 transition-all duration-200 hover:bg-white/[0.06] hover:text-foreground focus-visible:outline-2 focus-visible:outline-ring focus-visible:outline-offset-2"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="hidden md:flex items-center gap-3">
          <Button variant="ghost" size="sm" className="rounded-full px-4 text-foreground/60 hover:bg-white/[0.05] hover:text-foreground">
            Login
          </Button>
