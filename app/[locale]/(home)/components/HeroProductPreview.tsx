...
export default function HeroProductPreview() {
  return (
    <div className="relative mx-auto w-full max-w-[460px] scale-[0.92] sm:scale-100">
      ...
      <motion.div
        className="ref-app-window absolute -right-1 -top-2 z-20 w-[158px] scale-[0.94] shadow-2xl border border-[var(--qe-ref-card-border)]"
        initial={{ opacity: 0, y: -10 }}
        animate={{ opacity: 1, y: [0, -3, 0] }}
        transition={{ opacity: { duration: 0.5, delay: 0.4 }, y: { duration: 4, repeat: Infinity, ease: "easeInOut", delay: 1 } }}
      >
      ...
      <div className="ref-app-window relative z-10">
        <div className="ref-app-header">
          ...
          <div className="ml-auto flex items-center text-[10px] font-medium tracking-[0.1em] text-[var(--qe-ref-text-muted)]">
            <motion.span
              className="mr-1.5 inline-block h-1.5 w-1.5 rounded-full bg-[var(--qe-ref-green)]"
        ...
        <div className="p-3">
          ...
          <div className="mt-2 ref-mini-chart rounded-md">
            <svg width="100%" height="58" viewBox="0 0 260 58" fill="none" xmlns="http://www.w3.org/2000/svg">
              <motion.path
                d="M4 44 L28 37 L52 39 L76 24 L100 28 L124 15 L148 20 L172 8 L196 13 L220 2 L244 5 L258 -1"
                stroke="var(--qe-ref-green)"
                strokeWidth="2.5"
                strokeLinecap="round"
      ...
      <div className="ref-app-window mt-3 z-10">
        ...
        <div className="divide-y divide-[var(--qe-ref-card-border)] text-[11px]">
          {[
            { label: 'Debrief • NQ Long', value: '+2.8R', className: 'ref-green font-medium tabular-nums' },
            { label: 'Pulse Score Update', value: '82 → 87', className: 'font-medium tabular-nums text-[var(--qe-ref-text)]' },
            { label: 'Sentinel Alert', value: '2 rules active', className: 'text-amber-400 font-medium' },
          ].map((item, i) => (
            <motion.div
              key={i}
              ...
              transition={{ duration: 0.3, delay: 0.6 + i * 0.1, ease: [0.22, 1, 0.36, 1] }}
            >
              <span className="text-[var(--qe-ref-text-muted)]">{item.label}</span>
