...
export default async function AboutPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <UnifiedPageShell widthClassName="max-w-[1280px]" className="py-8">
      <header className="mb-6 rounded-xl border border-white/[0.06] bg-white/[0.070] p-6">
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          ...
          <Link href={`/${locale}/support`} className="rounded-full border border-white/[0.06] px-4 py-2 text-xs font-semibold uppercase tracking-[0.12em] text-foreground/95 hover:bg-accent/50">
            Contact Support
          </Link>
        </div>
      </header>
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <Card className="border-white/[0.06] bg-white/[0.070]">
          <CardHeader>
        ...
        <Card className="border-white/[0.06] bg-white/[0.070] md:col-span-2">
          <CardHeader>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {founderSkills.map((skill, index) => (
                <Badge key={index} variant="secondary" className="border-white/[0.06] bg-secondary/30 text-sm py-1 px-2 flex items-center gap-1">
                  {skill.icon}
                  {skill.name}
                </Badge>
              ))}
