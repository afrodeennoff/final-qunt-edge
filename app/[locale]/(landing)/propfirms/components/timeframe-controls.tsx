...
export function TimeframeControls({ timeframeLabel, timeframeOptions }: TimeframeControlsProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      <Label htmlFor="timeframe-select" className="text-xs font-semibold tracking-wide text-foreground/88">
        {timeframeLabel}
      </Label>
      <Select value={currentTimeframe} onValueChange={handleTimeframeChange}>
        <SelectTrigger
          id="timeframe-select"
          className="w-[200px] border border-border/75 bg-card/90 text-foreground shadow-none backdrop-blur-sm hover:bg-card focus-visible:ring-1 focus-visible:ring-primary/55 focus-visible:ring-offset-0"
        >
          <SelectValue />
        </SelectTrigger>
        <SelectContent className="border border-border/75 bg-card/95 text-foreground shadow-xl">
