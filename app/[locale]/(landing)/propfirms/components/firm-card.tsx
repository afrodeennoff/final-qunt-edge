...
export function FirmCard({ locale, name, slug, stats, accountSizesCount }: FirmCardProps) {
  ...
  return (
    <Link href={`/${locale}/firm/${slug}`} className="block group">
      <article className="relative overflow-hidden rounded-xl border border-border/30 bg-background/0.14 transition-all duration-300 hover:-translate-y-1 hover:border-border/40 hover:bg-[oklch(0.65_0.22_260/0.03)]">
        {/* Top accent line */}
        ...
        <div className="px-5 pb-4">
          <span className="inline-flex h-9 w-full items-center justify-center rounded-full border border-border/20 text-xs font-medium text-muted-foreground transition-all duration-200 group-hover:border-border/30 group-hover:bg-muted/50 group-hover:text-foreground/95">
            View Details
