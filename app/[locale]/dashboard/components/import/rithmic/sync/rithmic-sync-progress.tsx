...
export function RithmicSyncFeedback({ totalAccounts }: RithmicSyncFeedbackProps) {
 ...
 <motion.div
 key={accountId}
 className="flex flex-col gap-2 p-3 rounded-lg border bg-white/[0.02]"
 ...
 {latestMessage && (
 <div className={cn("text-xs mt-1 truncate",
 latestMessage.message?.toLowerCase().includes('error') ?"text-semantic-error" :"text-muted-foreground"
 )}>
