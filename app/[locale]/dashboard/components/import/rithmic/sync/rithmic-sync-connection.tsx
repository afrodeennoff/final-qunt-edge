...
export function RithmicSyncConnection({ setIsOpen }: RithmicSyncConnectionProps) {
 ...
 await Promise.all(
 existingCredentials
 .filter(cred => cred.id !== dataToSave.id)
 .map(cred => clearRithmicData(cred.id))
 )

 await saveRithmicData(dataToSave)
 ...
 <div key={account.account_id} className="flex items-center gap-2 p-2 rounded hover:bg-background/35">
 <Checkbox
