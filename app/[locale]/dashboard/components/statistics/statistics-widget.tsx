    ...
    >
      <div className="grid h-full grid-cols-2">
          ...
          <div className={cn(
            ...
            <h3 className="font-terminal text-[10px] font-bold uppercase tracking-widest mb-1.5 text-foreground/60">{t('statistics.profitLoss.title')}</h3>
            <div className="flex-1 flex flex-col justify-center gap-0.5">
              {/* Profits */}
              <div className="flex justify-between items-center">
                <span className="text-foreground/70 text-xs">{t('statistics.profitLoss.profits')}</span>
                <span className="text-xs font-medium font-terminal metric-positive">{formatCurrency(grossWin)}</span>
              </div>

              {/* Losses */}
              <div className="flex justify-between items-center">
                <span className="text-foreground/70 text-xs">- {t('statistics.profitLoss.losses')}</span>
                <span className="text-xs font-medium metric-negative font-terminal">{formatCurrency(grossLosses)}</span>
              </div>

              {/* Fees */}
              <div className="flex justify-between items-center">
                <span className="text-foreground/70 text-xs">- {t('statistics.profitLoss.fees')}</span>
                <span className="text-xs font-medium text-foreground/70 font-terminal">{formatCurrency(cumulativeFees)}</span>
              </div>

              {/* Payouts */}
              <div className="flex justify-between items-center">
                <span className="text-foreground/70 text-xs">- {t('statistics.profitLoss.payouts')} ({nbPayouts})</span>
                <span className="text-xs font-medium text-foreground/70 font-terminal">{formatCurrency(totalPayouts)}</span>
              </div>
              ...
              {/* Net Result */}
              <div className="flex justify-between items-center">
                <span className="text-foreground/70 text-xs font-medium">{t('statistics.profitLoss.net')}</span>
                <span className={cn(
          ...
          <div className={cn(
            ...
            <h3 className="font-terminal text-[10px] font-bold uppercase tracking-widest mb-1.5 text-foreground/60">{t('statistics.performance.title')}</h3>
            <div className="flex-1 flex flex-col justify-center gap-1.5">
              <div className="flex justify-between items-center">
                <span className="text-foreground/70 text-xs">{t('statistics.performance.winRate')}</span>
                <span className="text-sm font-medium font-terminal metric-positive">{winRate}%</span>
              </div>
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-1">
                  <span className="text-foreground/70 text-xs">{t('statistics.performance.avgWin')}</span>
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger>
              ...
              {size !== 'tiny' && (
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-1">
                    <span className="text-foreground/70 text-xs">{t('statistics.performance.avgLoss')}</span>
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger>
                          <Info className="h-3 w-3 text-foreground/60" />
          ...
          <div className={cn(
            ...
            <h3 className="font-terminal text-[10px] font-bold uppercase tracking-widest mb-1.5 text-foreground/60">{t('statistics.activity.title')}</h3>
            <div className="flex-1 flex flex-col justify-center gap-1.5">
              <div className="flex justify-between items-center">
                <span className="text-foreground/70 text-xs">{t('statistics.activity.totalTrades')}</span>
                <span className="text-sm font-medium text-foreground font-terminal">{nbTrades}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-foreground/70 text-xs">{t('statistics.activity.winningTrades')}</span>
                <span className="text-sm font-medium text-foreground font-terminal">{nbWin}</span>
              </div>
              {size !== 'tiny' && (
                <div className="flex justify-between items-center">
                  <span className="text-foreground/70 text-xs">{t('statistics.activity.avgDuration')}</span>
                  <span className="text-sm font-medium text-foreground font-terminal">{averagePositionTime}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className={cn(
            ...
            <h3 className="font-terminal text-[10px] font-bold uppercase tracking-widest mb-1.5 text-foreground/60">{t('statistics.distribution.title')}</h3>
            <div className="flex-1 flex flex-col justify-center gap-1.5">
              <div className="space-y-1">
                <div className="flex justify-between items-center">
                  <span className="text-foreground/70 text-xs">{t('statistics.distribution.long')}</span>
                  <span className="text-sm font-medium font-terminal metric-positive">{longRate}%</span>
                </div>
                <Progress value={longRate} className="h-1 bg-card/10" indicatorClassName="bg-primary/80 chart-positive-emphasis" />
              </div>
              {size !== 'tiny' ? (
                <>
                  <div className="space-y-1">
                    <div className="flex justify-between items-center">
                      <span className="text-foreground/70 text-xs">{t('statistics.distribution.short')}</span>
                      <span className="text-sm font-medium font-terminal metric-negative">{shortRate}%</span>
                    </div>
                     <Progress value={shortRate} className="h-1 bg-card/10" indicatorClassName="bg-destructive/70 chart-negative-muted" />
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-foreground/70 text-xs">{t('statistics.distribution.winningStreak')}</span>
