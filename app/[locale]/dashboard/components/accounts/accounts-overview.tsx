...
function SortRuleItem({
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        'type-body-sm flex items-center gap-2 rounded-md border border-border/20 bg-gradient-to-br from-card/50 to-card/10 px-3 py-2 text-foreground',
...
function DraggableAccountCard({
  ...
  return (
    <div ref={setNodeRef} style={style} className={cn('h-full', isDragging && 'z-50')}>
...
function PayoutDialog({
  ...
  const [, setDateInputValue] = useState<string>(() => format(initialDate, 'yyyy-MM-dd'))
  const t = useI18n()
  ...
  const statusOptions = [
    {
      value: 'PENDING',
      label: t('propFirm.payout.statuses.pending'),
      icon: <Clock className="h-4 w-4" />,
    },
    {
      value: 'VALIDATED',
      label: t('propFirm.payout.statuses.validated'),
      icon: <CheckCircle className="h-4 w-4" />,
    },
    {
      value: 'REFUSED',
      label: t('propFirm.payout.statuses.refused'),
      icon: <XCircle className="h-4 w-4" />,
    },
    {
      value: 'PAID',
      label: t('propFirm.payout.statuses.paid'),
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="flex h-[100dvh] w-full max-h-[100dvh] max-w-full flex-col pb-[max(env(safe-area-inset-bottom),0.75rem)] sm:w-[540px] sm:max-w-[540px]">
        <SheetHeader className="shrink-0">
          <SheetTitle>
            {existingPayout ? t('propFirm.payout.edit') : t('propFirm.payout.add')}
          </SheetTitle>
          <SheetDescription>
            {existingPayout
              ? t('propFirm.payout.editDescription')
              : t('propFirm.payout.addDescription')}{' '}
            {accountNumber}
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="flex-1 overflow-y-auto gap-6 py-6 min-h-0">
          ...
          <div className="space-y-3">
            ...
            <div className="flex max-h-[400px] flex-col rounded-xl border border-border/20 bg-gradient-to-br from-card/50 to-card/10 ring-1 ring-inset ring-white/[0.02]">
              ...
              <div className="p-2 flex-1 overflow-y-auto">
                <Calendar
                  ...
                  classNames={{
                    ...
                    cell: 'relative h-8 w-8 p-0 text-center text-xs focus-within:relative focus-within:z-20 [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-outside)]:bg-accent/50 [&:has([aria-selected])]:bg-accent/40 first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md',
                    day: 'h-8 w-8 rounded-md p-0 text-xs font-normal transition-colors hover:bg-accent/40 hover:text-accent-foreground aria-selected:opacity-100',
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    {t('propFirm.payout.deleteConfirm')} ${existingPayout.amount.toFixed(2)} on{' '}
                    {format(existingPayout.date, 'PP', { locale: dateLocale })}
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
...
function AccountsOverviewComponent({
  ...
  const { accountNumbers } = useDashboardFilters()
  const { deletePayout, saveAccount, savePayout } = useDashboardActions()
  ...
  const { filteredAccounts, unconfiguredAccounts } = useMemo(() => {
    ...
    const hiddenAccountNumbers = hiddenGroup
      ? new Set(hiddenGroup.accounts.map((a) => a.number))
      : new Set()
  ...
  const { sortedGroupEntries, sortedUngroupedAccounts } = useMemo(() => {
    ...
    if (groupSortRule) {
      groupEntries.sort((a, b) => compareSortValues(a.group.name, b.group.name, groupSortRule.desc))
  ...
  const handleAddPayout = async (payout: Payout) => {
    ...
    try {
      ...
      toast.success(
        selectedPayout ? t('propFirm.payout.updateSuccess') : t('propFirm.payout.success'),
        {
          description: selectedPayout
            ? t('propFirm.payout.updateSuccessDescription')
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const accountUpdate = {
        ...
        drawdownThreshold:
          pendingChanges?.drawdownThreshold ?? selectedAccountForTable.drawdownThreshold,
        consistencyPercentage:
          pendingChanges?.consistencyPercentage ?? selectedAccountForTable.consistencyPercentage,
        propfirm: pendingChanges?.propfirm ?? selectedAccountForTable.propfirm,
        resetDate:
          'resetDate' in pendingChanges
            ? pendingChanges.resetDate instanceof Date
              ? pendingChanges.resetDate
              : null
            : selectedAccountForTable.resetDate,
        shouldConsiderTradesBeforeReset:
          pendingChanges?.shouldConsiderTradesBeforeReset ??
          selectedAccountForTable.shouldConsiderTradesBeforeReset ??
          true,
        trailingDrawdown:
          pendingChanges?.trailingDrawdown ?? selectedAccountForTable.trailingDrawdown,
        trailingStopProfit:
          pendingChanges?.trailingStopProfit ?? selectedAccountForTable.trailingStopProfit,
        ...
        tradingNewsAllowed:
          pendingChanges?.tradingNewsAllowed ?? selectedAccountForTable.tradingNewsAllowed,
        ...
        minTradingDaysForPayout:
          pendingChanges?.minTradingDaysForPayout ??
          selectedAccountForTable.minTradingDaysForPayout,
        groupId:
  ...
  return (
    <Card
      className={cn(
        'group relative flex h-full w-full flex-col overflow-hidden rounded-xl border border-border/20 bg-gradient-to-br from-card/50 to-card/10 ring-1 ring-inset ring-white/[0.02] transition-all',
        surface === 'embedded' && 'opacity-95',
      )}
    >
      <CardHeader
        className={cn(
          'shrink-0 border-b border-border/20 bg-gradient-to-br from-card/40 to-card/10 px-4 py-3 sm:px-5',
          size === 'small' && 'px-3 py-3',
        )}
      >
        <div className="flex w-full flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex items-center gap-2">
            <CardTitle size={size === 'small' ? 'md' : 'xl'} className="line-clamp-1">
              {t('propFirm.title')}
            </CardTitle>
            <TooltipProvider>
              <UITooltip>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Button
              ...
              className={cn('gap-1.5', size === 'small' ? 'h-8 px-3 text-xs' : 'h-9')}
            >
            ...
            <Popover open={sortingMenuOpen} onOpenChange={setSortingMenuOpen}>
              ...
              <PopoverContent align="end" className="w-80 p-3">
                <div className="space-y-3">
                  {sorting.length === 0 ? (
                    <div className="text-sm text-muted-foreground">{t('table.noSorting')}</div>
                  ) : (
                    <DndContext
                      sensors={sensors}
                      ...
                      onDragEnd={(event: DragEndEvent) => {
                        ...
                        const oldIndex = sorting.findIndex((rule) => rule.id === active.id)
                        const newIndex = sorting.findIndex((rule) => rule.id === over.id)
                    ...
                    >
                      ...
                      >
                        <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                          {sorting.map((rule) => {
                            const label =
                              sortOptions.find((option) => option.id === rule.id)?.label ?? rule.id
                            return (
                              <SortRuleItem
                                key={rule.id}
                                sort={rule}
                                ...
                                toggleLabel={
                                  rule.desc ? t('table.sortDescending') : t('table.sortAscending')
                                }
                                removeLabel={t('table.removeSort')}
                                onToggleDirection={() =>
                                  setSorting((prev) =>
                                    prev.map((item) =>
                                      item.id === rule.id ? { ...item, desc: !item.desc } : item,
                                    ),
                                  )
                                }
                                onRemove={() =>
                                  setSorting((prev) => prev.filter((item) => item.id !== rule.id))
                                }
                              />
                            )
                          })}
                  ...
                  <div className="flex items-center gap-2">
                    <Select
                      ...
                      onValueChange={(value) => {
                        ...
                        if (nextValue) {
                          setSorting((prev) => [...prev, { id: nextValue, desc: false }])
                          setPendingSortId('')
                        }
                      }}
                    >
            ...
            <Tabs value={view} onValueChange={(value) => setView(value as 'cards' | 'table')}>
              <TabsList className={cn('gap-1', size === 'small' ? 'h-8 px-1' : 'h-9 px-1')}>
                <TabsTrigger
                  value="cards"
                  className={cn('gap-1.5', size === 'small' ? 'h-7 px-3 text-xs' : 'h-8')}
                >
      ...
      {/* Unconfigured accounts banner */}
       {unconfiguredAccounts.length > 0 && !isLoading && (
          <div className="border-b border-border/20 bg-gradient-to-br from-card/40 to-card/10 px-4 py-3 sm:px-5">
          ...
          {filteredAccounts.length === 0 && unconfiguredAccounts.length === 0 && !isLoading ? (
            ...
            <div className="space-y-6 px-3 py-4 sm:px-4 sm:py-5">
              {sortedGroupEntries.map(({ group, accounts: orderedAccounts }) => (
                <section
                  key={group.id}
                  className="group relative overflow-hidden rounded-xl border border-border/20 bg-gradient-to-br from-card/50 to-card/10 ring-1 ring-inset ring-white/[0.02] transition-all"
                >
                  <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/20 bg-gradient-to-br from-card/40 to-card/10 px-4 py-3">
          ...
          <DialogContent className="flex max-h-[calc(100dvh-1rem)] h-[min(80dvh,60rem)] w-[calc(100vw-1rem)] max-w-[calc(100vw-1rem)] flex-col overflow-y-auto pb-[max(env(safe-area-inset-bottom),0.75rem)] sm:w-full sm:max-w-7xl 2xl:max-w-[min(96vw,1600px)] 3xl:max-w-[1800px]">
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          {t('propFirm.delete.description', {
                            account: selectedAccountForTable?.number,
                          })}
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
            ...
            <div className="p-6 pt-4 flex-1 overflow-y-auto">
              {selectedAccountForTable && (
                <Tabs
                  defaultValue={
                    selectedAccountForTable.profitTarget === 0 ? 'configurator' : 'table'
                  }
                  className="w-full"
                >
                  ...
                  <TabsContent value="table" className="mt-4">
                    <AccountTable
                      ...
                      resetDate={
                        selectedAccountForTable.resetDate
                          ? new Date(selectedAccountForTable.resetDate)
                          : undefined
                      }
