...
export function TradeImageEditor({ trade, tradeIds }: TradeImageEditorProps) {
 ...
 />
 {imageArray.length > 1 && (
 <span className="absolute bottom-1 right-1 bg-background/0.14 text-foreground text-xs px-1 rounded">
 {imageArray.length}
 </span>
 )}
 </>
 ...
 variant="ghost"
 size="icon"
 className="bg-linear-to-r bg-primary/[0.03] hover:bg-accent/70 shadow-[0_4px_16px_-8px_rgba(0,0,0,0.5)] border-0 ring-1 ring-transparent/24 h-7 w-7 sm:h-8 sm:w-8"
 onClick={() => zoomOut()}
 disabled={scale <= 0.5}
 >
