    ...
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          size="sm"
          className={cn(
            '-ml-3 h-8 data-[state=open]:bg-accent/40',
            hasActiveFilter && 'bg-accent/40',
          )}
        >
          <span>{t('trade-table.tags')}</span>
          {hasActiveFilter && <Filter className="ml-1 h-3.5 w-3.5 text-muted-foreground" />}
        </Button>
      </DropdownMenuTrigger>
                ...
                <ScrollArea className="h-full">
                  <div className="gap-1">
                      ...
                      >
                        <div className="flex items-center gap-2 min-w-0 flex-1">
                          ...
                          <label
                            ...
                            className="type-body-sm flex-1 cursor-pointer truncate font-medium"
                          >
                            {tag.name}
                          </label>
                        </div>
                    ...
                    {filteredTags.length === 0 && (
                      <div className="type-body-sm flex h-[100px] items-center justify-center text-muted-foreground">
...
function TradeTableReviewComponent({ tradesParam, config }: TradeTableReviewProps) {
    ...
    trades.forEach((trade) => {
      ...
      const roundDate = (date: Date) => {
        ...
        roundedDate.setSeconds(
          Math.floor(date.getSeconds() / groupingGranularity) * groupingGranularity,
        )
  ...
  const getAllTradeIds = useMemo(() => {
    return groupedTrades.flatMap((row) => {
      ...
      const groupId = row.id && row.id !== '' ? [row.id] : []
  ...
  const allColumns = useMemo<ColumnDef<ExtendedTrade>[]>(
    () => [
      ...
      },
      {
        id: 'accounts',
        header: () => (
          <Button variant="ghost" size="sm" className="-ml-3 h-8 data-[state=open]:bg-accent/40">
          ...
          return (
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1">
                <Popover>
                  <PopoverTrigger asChild>
                    <div className="type-label flex h-6 min-w-6 w-fit cursor-pointer items-center justify-center rounded-full bg-primary/10 px-2 hover:bg-accent/60">
                      {accounts.length === 1
              ...
              {(trade.trades?.length ?? 0) > 0 && (
                <span className="text-xs text-muted-foreground">({trade.trades?.length})</span>
              )}
            </div>
          )
        },
      ...
      },
      {
        ...
        cell: ({ row }) =>
          formatInTimeZone(new Date(row.original.entryDate), timezone, 'yyyy-MM-dd'),
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const trade = row.original
          const tradeIds =
            (trade.trades?.length ?? 0) > 0 ? trade.trades!.map((t) => t.id) : [trade.id]
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return <div className="tabular-nums">{parsePositionTime(timeInPosition)}</div>
        },
        sortingFn: (rowA, rowB, columnId) => {
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <EditableTimeCell
              value={
                trade.entryDate ? new Date(trade.entryDate).toISOString() : new Date().toISOString()
              }
              tradeIds={tradeIds}
              fieldType="entryDate"
              onUpdate={updateTrades}
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <EditableTimeCell
              value={
                trade.closeDate ? new Date(trade.closeDate).toISOString() : new Date().toISOString()
              }
              tradeIds={tradeIds}
              fieldType="closeDate"
              onUpdate={updateTrades}
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataTableColumnHeader
            column={column}
            title={t('calendar.modal.commission')}
            tableId="trade-table"
          />
        ),
        cell: ({ row }) => {
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="text-right font-medium tabular-nums">{quantity.toLocaleString()}</div>
          )
        },
        sortingFn: 'basic',
        size: 100,
    ...
    ? {
        height:
          typeof config.style.height === 'number'
            ...
            : config.style.height,
        width:
          typeof config.style.width === 'number' ? `${config.style.width}px` : config.style.width,
      }
    : {}
  ...
  const endIndex = shouldVirtualizeRows
    ...
    : tableRows.length
  const visibleRows = shouldVirtualizeRows ? tableRows.slice(startIndex, endIndex) : tableRows
        ...
        <CardHeader className="shrink-0 border-b border-border bg-muted px-4 py-4 sm:px-6">
          <div className="flex w-full flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {selectedTrades.length > 0 && (
                <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
                  ...
                  <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                      <AlertDialogCancel>{t('trade-table.deleteConfirmCancel')}</AlertDialogCancel>
              ...
              {!config?.disableColumnConfig && <ColumnConfigDialog tableId="trade-table" />}
            </div>
          </div>
        </CardHeader>
      )}
      ...
      >
        {isMobile ? (
          ...
          <table className="type-body-sm w-full table-auto border-separate border-spacing-0 caption-bottom tabular-nums">
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                ...
                >
                  {headerGroup.headers.map((header) => (
                    <th
                      key={header.id}
                      className="type-label h-11 whitespace-nowrap border-r border-border/20 px-3.5 py-2.5 text-left align-middle text-muted-foreground last:border-r-0 first:border-l [&:has([role=checkbox])]:pr-0"
                    ...
                    return (
                      <React.Fragment key={row.id}>
                        <tr
                          ...
                          className={cn(
                            'group border-b border-border/20 transition-all duration-200 hover:bg-primary/[0.02]',
                            row.getIsSelected() &&
                        ...
                        >
                          {row.getVisibleCells().map((cell) => (
                            <td
                              key={cell.id}
                              className={cn(
                                'whitespace-nowrap border-r border-border/20 px-3.5 py-2 text-sm align-middle first:border-l last:border-r-0 group-hover:border-border/60 [&:has([role=checkbox])]:pr-0',
            ...
            <tfoot className="sticky bottom-0 z-10 border-t border-border/20 bg-gradient-to-br from-card/30 to-card/5 backdrop-blur-sm">
              <tr className="border-b transition-colors">
                {visibleColumns.map((column, index) => {
                  ...
                  const firstDataColumnIndex = visibleColumns.findIndex((col) => {
