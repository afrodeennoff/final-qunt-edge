...
const ChatWidget = React.memo(function ChatWidget({ size ="large" }: ChatWidgetProps) {
     ...
     onError: (error) => {
     logger.error({ error }, "Chat error occurred");
     },
     });
     ...
     {error && errorDetails && (
     <motion.div
     className="p-4 rounded-lg wrap-break-word overflow-hidden bg-destructive/10 text-destructive border border-destructive/20 mb-4"
     ...
     {errorDetails.detail && (
     <p className="text-xs text-muted-foreground mt-1">{errorDetails.detail}</p>
     )}
     </div>
     <Button
     type="button"
     ...
     if (
     chartData &&
     typeof chartData ==="object" &&"chartData" in chartData
     ) {
     return (
     ...
     <div className="absolute inset-0 bg-background/60 z-50 flex items-center justify-center p-4">
