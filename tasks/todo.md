...
# Task: Dashboard widget/runtime fixes (2026-03-18)

- [x] Replace `Date.now()` widget IDs in both dashboard context providers with collision-safe IDs.
- [x] Guard the daily tick target date sync so it only updates state when the derived date actually changes.
- [x] Simplify the widget canvas outside-click listener so it uses a stable callback dependency chain.
- [x] Hoist or memoize chart tooltip renderers in `tick-distribution.tsx` and `time-in-position.tsx` to avoid remount churn.
- [x] Replace the requested `CartesianGrid` opacity class pattern with explicit `stroke` / `strokeOpacity` props in the target chart files.
- [x] Run targeted lint/typecheck on the touched dashboard files and record the output.

Verification:
- `npx eslint 'app/[locale]/dashboard/dashboard-context.tsx' 'app/[locale]/dashboard/dashboard-context-auto-save.tsx' 'app/[locale]/dashboard/components/widget-canvas.tsx' 'app/[locale]/dashboard/components/charts/daily-tick-target.tsx' 'app/[locale]/dashboard/components/charts/tick-distribution.tsx' 'app/[locale]/dashboard/components/charts/time-in-position.tsx' 'app/[locale]/dashboard/components/charts/weekday-pnl.tsx' 'app/[locale]/dashboard/components/charts/pnl-time-bar-chart.tsx' 'app/[locale]/dashboard/components/charts/pnl-per-contract-daily.tsx' 'app/[locale]/dashboard/components/charts/contract-quantity.tsx' 'app/[locale]/dashboard/components/charts/pnl-bar-chart.tsx' 'app/[locale]/dashboard/components/charts/time-range-performance.tsx' 'app/[locale]/dashboard/components/charts/pnl-per-contract.tsx' 'app/[locale]/dashboard/components/charts/equity-chart.tsx'` -> pass with warnings only.
