...
const _getActiveDeals = async (): Promise<DealItem[]> => {
  ...
  try {
    ...
    return coupons.map((coupon) => ({
      ...
      platform: (coupon.platform || coupon.propFirm.platform || 'Tradovate') as TradingPlatform,
      payoutModel: (coupon.payoutModel || coupon.propFirm.payoutModel || 'Monthly') as PayoutModel,
      drawdownType: (coupon.drawdownType || coupon.propFirm.drawdownType || 'Static') as DrawdownType,
      discountPercent: coupon.discountPercent ?? 0,
